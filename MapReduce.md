...
#### 自定义InputFormat:
##### 决定以什么形式输入,这时候map形参要注意对应
1. dirver job.set
2. 创建自定义的inputFormat extends FileInputFormat
3. 实现对应方法 recordreader 除了要创建对应的recordreader 还需要通过
  initialize(spilt, context)来进行赋值
4. 创建对应的recordreader extends recordreader
5. 实现对应的方法
   * initialaize(InputSplit split, TaskAttemptContext context)因为我们recordreader时真正要读取
数据的人,所以要通过split来得到要读取文件的信息,从context.getConfiguration() 方法中获取我们配置
文件系统的位置
   * nextKeyValue() 按格式读取文件,并根据读取的结果返回,返回值将决定map是否执行.
   * getCurrentKey() map方法传入的key值是从这个方法中获取
   * getCurrentValue() map方法传入的value值是从这个方法中获取
   * getProgress() 获取进度的方法
...
3. 实现对应方法	RecordWriter
   * 需要通过this.getOutputPath(job);拿到我们想要输出的位置
   * 根据输出目录创建对应的输出流,传给recordWriter
4. 创建对应的RecordWriter extends RecordWriter
5. 实现三个方法
   * 带参构造用来接收输出流,知道要在哪些位置写
   * writer方法能够接收到最终要写入的内容,在这进行逻辑判断决定往哪个输出流进行写入.
   * close方法用来关闭流,也是我们job的结束
