...
1. 用hadooponwindows-master中的bin和etc文件夹把hadoop-2.7.3中的相同文件夹替换掉.
2. 把hadoop-2.7.3中的hadoop.dll文件和winutils.exe文件copy到电脑Windows目录下System32文件中
3. 配环境变量:配置java和hadoop的环境变量
4. 修改hadoop-2.7.3/etc/hadoop目录下的hadoop-env.cmd文件中的JAVA_HOME路径
(目录中文件夹名称中不能有空格)
5. 修改配置文件:修改hdfs-site文件中namenode和datanode的文件存储目录
